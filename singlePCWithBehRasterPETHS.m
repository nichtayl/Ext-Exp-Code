...
if sum(relevantPuffDurs>0 & isnan(relevantLaserTimes))>0
    for i = 1:length(puffDurs)
        ...
        [output.CSpkToPuff(i,1), output.CSpkToPuffChangeDir(i,1), output.CSpkToPuffProb(i,1), ...
            output.CSpkToPuffLat(i,1), output.SSToPuff(i,1), ...
            output.SSToPuffChangeDir(i,1), output.SSToPuffRate(i,1), output.SSToPuffLat(i,1), ...
            output.SSToPuffDev(i,1)] = ...
            gatherSpikeResponseMetrics(CSpk_times, SS_times, relevantTrialTimes,...
            USOnly.times+tubingDelay, 0.005, -0.2, 0.05, 1);
        
        if output.SSToPuff(i,1) || output.CSpkToPuff(i,1)
...
if sum(isnan(relevantPuffTimes) & relevantLaserDurs>0)>0
    for m = 1:length(laserAmps)
        ...
        [output.CSpkToLas(1,m), output.CSpkToLasChangeDir(1,m), output.CSpkToLasProb(1,m), ...
            output.CSpkToLasLat(1,m), output.SSToLas(1,m), ...
            output.SSToLasChangeDir(1,m), output.SSToLasRate(1,m), output.SSToLasLat(1,m), ...
            output.SSToLasDev(1,m)] = ...
            gatherSpikeResponseMetrics(CSpk_times, SS_times, relevantTrialTimes,...
            laserOnly.times, 0.005, -0.2, 0.05, 1);
        
        if output.SSToLas(1,m) || output.CSpkToLas(1,m)
            % SS and CSpk on puff alone trials
...
if sum(relevantPuffTimes>0 & relevantLaserTimes>0)>0
    for i = 1:length(puffDurs)
        for m = 1:length(laserAmps)
            ...
            [output.CSpkToLasPuff(i,m), output.CSpkToLasPuffChangeDir(i,m), output.CSpkToLasPuffProb(i,m), ...
            output.CSpkToLasPuffLat(i,m), output.SSToLasPuff(i,m), ...
            output.SSToLasPuffChangeDir(i,m), output.SSToLasPuffRate(i,m), output.SSToLasPuffLat(i,m), ...
            output.SSToLasPuffDev(i,m)]= ...
                gatherSpikeResponseMetrics(CSpk_times, SS_times, relevantTrialTimes,...
                USAndLaser.LTimes, 0.005, -0.2, 0.04, 1);
            
            % this analysis might be messed up for the SS if they have a
            % response to the laser alone...
            [output.CSpkToPuffLas(i,m), output.CSpkToPuffLasChangeDir(i,m), output.CSpkToPuffLasProb(i,m), ...
            output.CSpkToPuffLasLat(i,m), output.SSToPuffLas(i,m), ...
            output.SSToPuffLasChangeDir(i,m), output.SSToPuffLasRate(i,m), output.SSToPuffLasLat(i,m), ...
            output.SSToPuffLasDev(i,m)] = ...
                gatherSpikeResponseMetrics(CSpk_times, SS_times, relevantTrialTimes,...
