...
if sum(relevantPuffTimes>0 & relevantLaserTimes>0)>0
    for i = 1:length(puffDurs)
        for m = 1:length(laserAmps)
            ...
            [hasCSpkToLaserOnPuffTrials(1,m), CSpkChangeToLaserOnPuffTrialsDir(1,m), CSpkToLaserOnPuffTrialsProb(1,m), ...
                medianCSpkLatencyToLaserOnPuffTrials(1,m), hasSSToLaserOnPuffTrials(1,m), ...
                SSChangeToLaserOnPuffTrialsDir(1,m), SSToLaserOnPuffTrialsRate(1,m), SSRespToLaserOnPuffTrialsLat(1,m), ...
                SSRespToLaserOnPuffTrialsDeviation(1,m)] = ...
                gatherSpikeResponseMetrics(CSpk_times, SS_times, relevantTrialTimes,...
                USAndLaser.LTimes, 0.005, -0.2, 0.04, 1);
            
            % this one is goig to require some editing to account for the
            % fact that the trial's first stimulus may not be the event of
            % interest
            [hasCSpkToPuffOnLaserTrials(1,m), CSpkChangeToPuffOnLaserTrialsDir(1,m), CSpkToPuffOnLaserTrialsProb(1,m), ...
                medianCSpkLatencyToPuffOnLaserTrials(1,m), hasSSToPuffOnLaserTrials(1,m), ...
                SSChangeToPuffOnLaserTrialsDir(1,m), SSToPuffOnLaserTrialsRate(1,m), SSRespToPuffOnLaserTrialsLat(1,m), ...
                SSRespToPuffOnLaserTrialsDeviation(1,m)] = ...
                gatherSpikeResponseMetrics(CSpk_times, SS_times, relevantTrialTimes,...
